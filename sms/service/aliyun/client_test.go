...
func TestAliYunSMSClientSender_SendSms(t *testing.T) {
	//  初始化短信sender
	const (
		AccessKeyID     = "LTAI5tBxKmkodQRhrt5s4q1d"
		AccessKeySecret = "QuRHbS824lRSD4TxEGC2heFS8NrDtn"
	)
	credential := AliYunSMSClientCredential{
		AccessKeyId:     AccessKeyID,
		AccessKeySecret: AccessKeySecret,
	}
	sender, err := credential.CreateAliYunSMSClientSender("万有矩阵", AliYunSmsTemplate227130261{}, &AliYunSMSOptionalConfig{})
	if err != nil {
		t.Error(err.Error())
	}
	// 初始化短信sender
	...
	// 测试发送功能
	resp, err := sender.SendSms([]string{"13950318701"}, nil, nil, "666666")
	if err != nil {
		t.Error(err.Error())
	}
